...
const gridContainer = document.querySelector(".list");
// console.dir(gridContainer);

function showListBasic(val = "") {
  gridContainer.innerHTML = "";
  //여러개의 태그들을 리스트화
  wantedtags = val.split(" ");

  sibalBasic.forEach((pose) => {
    let flag = 0;
...
function showListCustom(val = "") {
  ...
  sibalCustom.forEach((pose) => {
    ...
    if (pose.tag.includes(wantedtag)) {
      ...
      li.draggable = "true";
      li.innerHTML = `<div class="posePictureBox" id="image${cnt}" ondragstart="drag(event)" style="background-image: url('${pose.url}');">
...
if (basicSearchInput == null) {
  customSearchBtn.addEventListener("click", (e) => {
    e.preventDefault();
    const val = customSearchInput.value;
    console.log(`val=${val}`);
    showListCustom(val);
  });
} else {
  basicSearchBtn.addEventListener("click", (e) => {
    e.preventDefault();
    const val = basicSearchInput.value;
    console.log(val);
    showListBasic(val);
  });
}
